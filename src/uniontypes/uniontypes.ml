module UnionTypes = Zoo.Main(struct
  ...
  type environment = (string * Syntax.kind) list * (string * Syntax.ty) list * Syntax.environment
  let options = []
  ...
  let rec exec (kctx, ctx, env) = function
    | Syntax.Expr e ->
    ...
    | Syntax.Def (x, e) ->
        ...
        (kctx, (x,ty) :: ctx, (x,v) :: env)
    | Syntax.TypeDecl (x, ty) ->
        (* type-evaluate [ty], and store it in the context! *)
        let ty_eval = Type_check.eval_type ctx ty in
        let kind = Type_check.kind_of kctx ty in
        Zoo.print_info "type %s : %s = %s@." x (Syntax.string_of_kind kind) (Syntax.string_of_type ty_eval);
        ((x, kind)::kctx, (x, ty_eval) :: ctx, env)
end) ;;
